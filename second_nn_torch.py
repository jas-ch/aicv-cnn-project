...
def clean_up(model, epoch) -> None:
    ...
    torch.save(model, "saves/nn2_model_" + str(epoch) + "_" + TIME_STAMP)
...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_shape: (int, int, int)):
        ...
        self.linear1 = torch.nn.Linear(
            in_features = 23232,  # 11 * 11 * 192 = 23232,
            out_features = 8192, 
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = self.linear4(y)
        if self.print:
            print(f"{'after 4th linear:':>30}", y.shape)
...
def save_code() -> None:
    with open(__file__, 'r') as f:
        this_code = f.read()
    with open("saves/nn2_code_save_" + TIME_STAMP + ".py", 'w') as f:
...
def main():
    ...
    output_file = open("saves/nn2_printout_" + TIME_STAMP + ".txt", "w")
    ...
    for i in range(epochs):
        ...
        # save
        PATH = f'saves/nn2_checkpoint_epoch{i+1}.tar'
        torch.save({
            'epoch': i,
