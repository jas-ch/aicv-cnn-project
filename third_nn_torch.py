...
def clean_up(model, epoch) -> None:
    ...
    torch.save(model, "saves/nn3_model_" + str(epoch) + "_" + TIME_STAMP)
...
def save_code() -> None:
    with open(__file__, 'r') as f:
        this_code = f.read()
    with open("saves/nn3_code_save_" + TIME_STAMP + ".py", 'w') as f:
...
def main():
    ...
    output_file = open("saves/nn3_printout_" + TIME_STAMP + ".txt", "w")
    ...
    for i in range(epochs):
        ...
        # save
        PATH = f'saves/nn3_checkpoint_epoch{i+1}.tar'
        torch.save({
            'epoch': i,
